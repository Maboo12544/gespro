...
export async function POST(request:Request){
 ...
 if(!r.ok){const error=await r.json() as {message?:string};const messages:Record<string,string>={'Configure bank lotteries and closing times first':'Admin dwe anrejistre konfigirasyon bank la ak lè fèmti yo.','Closing time not configured':'Lè fèmti lotri sa a poko konfigire.','Lottery closed':'Lotri sa a deja fèmen.','Lottery unavailable':'Lotri sa a pa disponib nan bank la.','Lottery rules unconfirmed':'Chwazi yon fòma tiraj valide pou lotri sa a.','Number limit exceeded':'Montan sa a depase limit boul la pou jounen an.','Invalid limit':'Admin dwe korije limit bank la.'};return fail(r.status>=500?503:r.status===403?403:400,messages[error.message||'']||(body.operation==='cancel'?'Anilasyon refize: dwa oswa delè a pa valab.':'Tikè refize. Verifye lotri, boul, montan ak dwa kont ou.'))}
