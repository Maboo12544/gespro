create table if not exists public.gespro_bank_configuration(
...
create or replace function gespro_private.stamp_bank_configuration() returns trigger language plpgsql security invoker set search_path='' as $$begin
...
drop trigger if exists stamp_bank_configuration on public.gespro_bank_configuration;
...
create or replace function public.gespro_save_bank_configuration(target_bank uuid,expected_version bigint,settings jsonb) returns public.gespro_bank_configuration language plpgsql security invoker set search_path='' as $$
