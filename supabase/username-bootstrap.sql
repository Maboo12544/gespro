alter table public.gespro_profiles add column username text unique check(username ~ '^[a-z0-9][a-z0-9._-]{2,31}$');
create table gespro_private.bootstrap (
...
create function public.gespro_bootstrap_allowed(token text) returns boolean language sql security invoker set search_path='' as $$
...
$$;
create function public.gespro_finish_bootstrap(token text, target_user uuid) returns void language plpgsql security invoker set search_path='' as $$
declare configured_username text;
begin
 select username into configured_username from gespro_private.bootstrap
 where token_hash=encode(sha256(convert_to(token,'UTF8')),'hex') and used_at is null and expires_at>now() for update;
 ...
 update gespro_private.bootstrap set used_at=now() where singleton=true and token_hash=encode(sha256(convert_to(token,'UTF8')),'hex') and used_at is null;
end $$;
