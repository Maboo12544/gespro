...
create table public.gespro_test_games(name text primary key,mode text not null check(mode in ('bolet3','pick2','massachusetts4','dominican3')));
...
create table public.gespro_test_tickets(
...
create index gespro_test_tickets_scope on public.gespro_test_tickets(bank_id,pos_id,created_at desc,id desc);
create table public.gespro_test_ticket_audit(
...
create function gespro_private.require_test_pos(target uuid) returns public.gespro_points_of_sale
...
create function gespro_private.create_test_ticket(target_pos uuid,request_key uuid,entries jsonb) returns public.gespro_test_tickets
language plpgsql security definer set search_path='' as $$
...
create function gespro_private.cancel_test_ticket(ticket_key bigint) returns public.gespro_test_tickets
language plpgsql security definer set search_path='' as $$
...
create function public.gespro_create_test_ticket(target_pos uuid,request_key uuid,entries jsonb) returns public.gespro_test_tickets language sql security invoker set search_path='' as $$select gespro_private.create_test_ticket(target_pos,request_key,entries)$$;
create function public.gespro_cancel_test_ticket(ticket_key bigint) returns public.gespro_test_tickets language sql security invoker set search_path='' as $$select gespro_private.cancel_test_ticket(ticket_key)$$;
