begin;
create function gespro_private.update_bank_entity(target_bank uuid,entity_id uuid,entity_kind text,title text,enabled boolean) returns void language plpgsql security definer set search_path='' as $$
...
create function public.gespro_update_bank_entity(target_bank uuid,entity_id uuid,entity_kind text,title text,enabled boolean) returns void language sql security invoker set search_path='' as $$select gespro_private.update_bank_entity(target_bank,entity_id,entity_kind,title,enabled)$$;
