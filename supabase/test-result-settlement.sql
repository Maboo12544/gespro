begin;
-- Test ledger only. No cash balances or disbursements are changed.
create table public.gespro_test_results (
...
create table public.gespro_test_result_audit (
...
alter table public.gespro_test_tickets drop constraint gespro_test_tickets_status_check;
...
alter table public.gespro_test_tickets add column prize_cents bigint not null default 0 check(prize_cents>=0),
 add column settlement jsonb not null default '[]', add column review_required boolean not null default false;

create function gespro_private.derive_test_result(mode text,numbers jsonb) returns jsonb
...
-- Freeze the effective tariff before assignment/configuration changes can affect a ticket.
create function gespro_private.snapshot_test_rates() returns trigger
...
-- Unconfirmed game semantics stay pending with an explicit review reason.
create function gespro_private.recalculate_test_ticket(ticket_key bigint) returns void
language plpgsql security definer set search_path='' as $$
 ...
 for e in select value from jsonb_array_elements(t.plays) loop
  ...
  for i in 0..(case when e->>'type' in ('DIRECTO','REVÈ','BOUL PÈ') then least(2,jsonb_array_length(r.derived)-1) else 0 end) loop
   ...
   if r.derived->>position=e->>'number' then
    ...
    amount=(e->>'amount')::numeric*rate::numeric*100;
...
create function gespro_private.publish_test_result(target_bank uuid,lottery_id text,result_date date,numbers jsonb,expected_version integer,reason text) returns public.gespro_test_results
language plpgsql security definer set search_path='' as $$
...
begin
 ...
 -- Every configured bank cutoff and historical ticket cutoff must have passed.
...
create function public.gespro_publish_test_result(target_bank uuid,lottery_id text,result_date date,numbers jsonb,expected_version integer,reason text) returns public.gespro_test_results
