...
{detail&&<aside className="monitor-detail"><header><h3><Localized text={"Detay tikè"}/></h3><Button variant="ghost" aria-label="Fèmen detay tikè" onClick={()=>{setSelected(null);setConfirm(false)}}>×</Button></header><h4>{detail.id}</h4><p><b>{detail.paidAt?"PAID":detail.status==="winner"?"WINNER":detail.status==="loser"?"LOSER":statusName(detail).toUpperCase()}</b></p>
      <p>{stamp(detail.createdAt)} — {detail.seller}</p><div className="monitor-detail-plays">{detail.plays.map((play,index)=><p key={index}><span>{play.lottery} • {play.type} • <b>{play.number}</b></span><strong>{money(play.amount)}</strong></p>)}</div><p>Bet: <b>{money(detail.amount)}</b> • Prize: <b>{money(detail.prize)}</b></p>{detail.paidAt&&<p>Paid: <b>{stamp(detail.paidAt)}</b> • {detail.paidBy||"—"}</p>}
