...
export function AccessManagement({initial}:{initial:AccessContext}){
 ...
 <div className="access-content">{access.isPlatformAdmin&&<a href="/demo">Demonstrasyon admin / pwen vant →</a>}
 ...
 </div>{opened&&bankItem&&points.some(p=>p.id===opened)&&(access.isPlatformAdmin||membership?.role==='owner'||membership?.role==='seller')&&<section><div className="access-test-banner"><b>Pwen vant — {points.find(p=>p.id===opened)?.name}</b><span></span><Button variant="outline" onClick={()=>setOpened('')}>Fèmen pwen vant</Button></div><DemoNamespaceProvider key={access.userId+bank+opened} scope={access.userId+':'+bank+':'+opened}><DemoStorageBoundary><ScopedTestPos userId={access.userId} bankId={bank} bank={bankItem.name} pointId={opened} seller={access.displayName||access.username}/></DemoStorageBoundary></DemoNamespaceProvider></section>}</main>
}
