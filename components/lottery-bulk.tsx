...
export function LotteryBulk({state,onChange,bank,management}:{state:LotteryState;onChange:(s:LotteryState)=>void;bank:string;management:BankManagement}){
 ...
 return <section className="lottery-payout">
  ...
  <div className="bank-point-list lottery-select-grid">{items.map(i=>{const region=i.name.includes("NEW YORK")?"NY":i.name.includes("GEORGIA")?"GA":i.name.includes("MASSACHUSETTS")?"MA":i.resultMode==="dominican3"?"RD":"FL";return <label key={i.id} className={"lottery-select-card "+(ids.includes(i.id)?"selected":"")}><Checkbox aria-label={"Chwazi "+i.name} checked={ids.includes(i.id)} onCheckedChange={v=>setSelection(old=>v===true?[...new Set([...old,i.id])]:old.filter(id=>id!==i.id))}/><span className={"lottery-region region-"+region.toLowerCase()}>{region}</span><span><b>{i.name}</b><small>{i.resultMode==="dominican3"?"3 boul":i.resultMode==="massachusetts4"?"4 chif":i.resultMode==="pick2"?"Pick 2":"Pick 3 / Pick 4"}</small></span></label>})}</div>
  ...
  <div className="lottery-config-tabs"><Button variant={tab==="limit"?"default":"outline"} onClick={()=>setTab("limit")}>2. Limit</Button><Button variant={tab==="payout"?"default":"outline"} onClick={()=>setTab("payout")}>3. Payout</Button></div>
  {tab==="payout"?<><p>Valè valide yo ranplase tarif pou tout lotri chwazi yo nan nivo sa a.</p>{ids.length>0?<PayoutSettings dominican={allRD} key={bank+scope+ids.join(",")} bank={bank+" • "+ids.length+" lotri chwazi"} rates={allRD?Object.fromEntries(Object.keys(defaultPayoutRates()).map(k=>[k,"0.00"])):defaultPayoutRates()} onSave={rates=>{const next={...state.rates};ids.forEach(id=>{next[JSON.stringify([id,bank,scope])]={...rates}});onChange({...state,rates:next});setMessage("Payout aplike sou "+ids.length+" lotri.")}}/>:<p>Chwazi yon lotri pou konfigire payout.</p>}</>:<>
   ...
   <div className="lottery-actions lottery-save-row"><Button variant="outline" onClick={()=>setValues(Array(7).fill(values[0]))}>Lendi → Tout jou</Button><Button className="admin-save-button" disabled={!ids.length} onClick={save}>💾 SAVE LIMIT</Button></div>
