...
export function DashboardReport({tickets,onCancel,serverBacked=false,canCancel=true}:{serverBacked?:boolean;canCancel?:boolean;tickets:MonitoredTicket[];onCancel:(id:string)=>boolean|Promise<boolean>}){
 ...
 <footer className="sales-pagination"><span>{points.length} POS • Paj {current}/{pages}</span><Button variant="outline" disabled={current===1} onClick={()=>setPage(current-1)}>Anvan</Button><Button variant="outline" disabled={current===pages} onClick={()=>setPage(current+1)}>Apre</Button></footer><p className="sales-control-note">Ouvri POS la pou wè tikè, detay jwèt ak enpresyon. Anilasyon disponib pou tikè an atant ki toujou nan delè yo; tras tikè a rete nan istorik la. Balans ak mouvman kès bank lan disponib nan tablo bank lan.</p></section>
