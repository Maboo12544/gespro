...
export function DemoCash({tickets,banks}:{tickets:MonitoredTicket[];banks:string[]}){
  const [bank,setBank]=useDemoState("demo-cash.tsx:bank",banks[0]||"JP Bòlèt");
  ...
  const [example,setExample]=useDemoState("demo-cash.tsx:example",false);
  ...
  if(!sellers.length)sellers.push("Vandè tès");
  const selected=sellers.includes(seller)?seller:sellers[0];
  const key=JSON.stringify([bank,selected,date,example]);
  ...
  const gross=example?100000:rows.reduce((s,t)=>s+Math.round(t.amount*100),0);
  const cancelled=example?10000:rows.filter(t=>t.status==="cancelled").reduce((s,t)=>s+Math.round(t.amount*100),0);
  const paid=example?15000:rows.filter(t=>t.status!=="cancelled"&&t.paidAt).reduce((s,t)=>s+Math.round(t.prize*100),0);
  ...
  function exportReport(){
    const lines=[["GESPRO — RAPÒ KÈS"],["Bank",bank],["Vandè",selected],["Dat",date],["Sous",example?"Egzanp separe":"Tikè sesyon an"],["Lavant brit",money(summary.gross)],["Anilasyon",money(summary.cancelled)],["Lavant nèt",money(summary.net)],["Komisyon",money(summary.commission)],["Gayan peye",money(summary.paid)],["Kòb remèt",money(summary.remitted)],["Balans",money(summary.balance)],["Estati",closed?"Fèmen — foto kalkil lè fèmti":"Ouvè"]];
  ...
  return <section className="panel demo-cash-panel">
    ...
    <div className="demo-cash-controls">
      ...
      <label>Sous kalkil<select value={example?"example":"tickets"} onChange={e=>{setExample(e.target.value==="example");setError("");}}><option value="tickets">Tikè tès sesyon an</option><option value="example">Egzanp separe: $1,000 lavant</option></select></label>
    </div>
    {example&&<p>Egzanp: $1,000 lavant, $100 anile ak $150 gayan peye. Li pa ajoute okenn tikè nan pwen vant la.</p>}
    ...
    <div className="table-scroll"><table><thead><tr><th><Localized text={"Lè"}/></th><th>Remiz tès</th></tr></thead><tbody>{ledger.entries.map(e=><tr key={e.id}><td>{e.at}</td><td>{money(e.amount)}</td></tr>)}{!ledger.entries.length&&<tr><td colSpan={2}>Pa gen remiz pou seleksyon sa a.</td></tr>}</tbody></table></div>
    ...
    <h3>Istorik fèmti</h3><div className="table-scroll"><table><thead><tr><th><Localized text={"Bank"}/></th><th><Localized text={"Vandè"}/></th><th><Localized text={"Jou"}/></th><th><Localized text={"Balans"}/></th></tr></thead><tbody>{closings.filter(c=>c.bank===bank).map(c=><tr key={c.id}><td>{c.bank}</td><td>{c.seller}</td><td>{c.date}{JSON.parse(c.id)[3]?" (egzanp)":""}</td><td>{money(c.summary.balance)}</td></tr>)}{!closings.some(c=>c.bank===bank)&&<tr><td colSpan={4}>Pa gen fèmti pou bank sa a.</td></tr>}</tbody></table></div>
  </section>;
}
