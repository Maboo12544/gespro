...
export function DrawResults({state,onChange}:{state:LotteryState;onChange:(s:LotteryState)=>void}){
 ...
 const [base,setBase]=useState('');const [apiBusy,setApiBusy]=useState(false);const [autoApi,setAutoApi]=useState(false);
 ...
 useEffect(()=>{if(!autoApi||!item||pending)return;void loadApi()},[autoApi,lottery,date]);
