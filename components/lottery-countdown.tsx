...
export function LotteryCountdown({schedule}:{schedule?:ClosingSchedule}){
 const [now,setNow]=useState(()=>new Date());
 useEffect(()=>{if(!schedule)return;const id=setInterval(()=>setNow(new Date()),1000);return()=>clearInterval(id)},[schedule]);
 const remaining=schedule?secondsToClose(schedule,now):null;
 const soon=remaining!==null&&remaining>0&&remaining<=300;
 return <small className={"lottery-close-time"+(soon||remaining!==null&&remaining<=0?" closing-soon":"")} title={schedule?`${schedule.zone} — chak jou`:"Mete lè fèmti nan Admin → Lotri"}>{remaining===null?"Lè fèmti: —":remaining<=0?`Fèmen · ${schedule!.time}`:soon?`Fèmen nan ${String(Math.floor(remaining/60)).padStart(2,"0")}:${String(remaining%60).padStart(2,"0")}`:`Fèmen ${schedule!.time}`}</small>
}
