...
export function LotteryCountdown({schedule,serverOffsetMs=0}:{schedule?:ClosingSchedule;serverOffsetMs?:number}){
 ...
 return <small className={"lottery-close-time"+(soon||remaining!==null&&remaining<=0?" closing-soon":"")} title={schedule?`${schedule.zone} — chak jou`:"Mete lè fèmti nan Admin → Lotri"}>{remaining===null?"Lè fèmti: —":remaining<=0?`Fèmen · ${schedule!.time}`:`Fèmen nan ${String(Math.floor(remaining/60)).padStart(2,"0")}:${String(remaining%60).padStart(2,"0")}`}</small>
}
