...
export function LotteryCountdown({schedule,serverOffsetMs=0}:{schedule?:ClosingSchedule;serverOffsetMs?:number}){
 ...
 return <small className={"lottery-close-time"+(soon||remaining!==null&&remaining<=0?" closing-soon":"")} title={schedule?`${schedule.zone} — vant fèmen 8 minit anvan tiraj`:"Mete lè fèmti nan Admin → Lotri"}>{remaining===null?"Closing: —":remaining<=0?"Closed":`Closing · ${String(Math.floor(remaining/60)).padStart(2,"0")}:${String(remaining%60).padStart(2,"0")}`}</small>
}
