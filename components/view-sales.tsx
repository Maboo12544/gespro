...
export function ViewSales({open,onOpenChange,tickets,lotteries=[],results=[],bank="JP Bòlèt",pointId="jp-pos-1"}:{bank?:string;pointId?:string;results?:(import("@/components/draw-results").DrawResult&{name:string})[];lotteries?:string[];open:boolean;onOpenChange:(v:boolean)=>void;tickets:MonitoredTicket[]}){
 ...
 const report=<><h2><Localized text={"Rezime lavant — "}/>{selected}</h2><p><Localized text={"Done ki disponib nan sesyon sa a."}/></p>{tab==="sales"?<><div className="sales-balance-banner"><Localized text="Balans aktyèl"/>: —</div><div className="sales-pending-banner"><Localized text="Montan tikè an atant"/>: {money(pendingAmount)}</div><table className="sales-summary"><tbody>{rows.map(([k,v])=><tr key={k}><th><Localized text={k}/></th><td><Localized text={v}/></td></tr>)}</tbody></table><h3><Localized text={"Total pa lotri"}/></h3>{table(["Lotri","Total vann","Komisyon","Pri","Nèt"],[...groups].length?[...groups].map(([name,total])=><tr key={name}><td><button className="monitor-serial" onClick={()=>setDetail(name)}>{name}</button></td><td>{money(total)}</td><td>—</td><td>—</td><td>—</td></tr>):empty(5))}<h3><Localized text="Tikè genyen"/></h3>{winners(active.filter(t=>t.status==="winner"))}<h3><Localized text="Nimewo genyen"/></h3><p>— vle di rezilta poko antre.</p>{resultTable(names)}</>:<><h3><Localized text={"Tranzaksyon resan"}/></h3><p>Lis anba a montre tranzaksyon lavant ki disponib pou dat la.</p><table><thead><tr>{["Kalite","Dat","Referans","Montan","Estati"].map(x=><th key={x}><Localized text={x}/></th>)}</tr></thead><tbody>{day.map(t=><tr key={t.id}><td><Localized text={"Lavant"}/></td><td>{new Date(t.createdAt).toLocaleString("fr-FR")}</td><td>{t.id}</td><td>{money(t.amount)}</td><td>{t.status==="cancelled"?"Anile":"Anrejistre"}</td></tr>)}</tbody></table>{!day.length&&<p><Localized text={"Pa gen tranzaksyon pou dat sa a."}/></p>}</> }</>;
