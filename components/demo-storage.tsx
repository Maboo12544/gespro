...
export function StorageNotice(){const [failed,setFailed]=useState(false);useEffect(()=>{const handler=()=>setFailed(true);window.addEventListener('gespro-storage-error',handler);return ()=>window.removeEventListener('gespro-storage-error',handler)},[]);return failed?<div className="storage-warning" role="alert">Navigatè a pa ka konsève travay la. Pa rafrechi anvan ou telechaje rapò ou.</div>:null}
