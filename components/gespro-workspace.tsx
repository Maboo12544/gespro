...
function AdminDashboard({ setView, tickets, onCancel, lotteryState, setLotteryState }: { onCancel: (id: string) => boolean; setView: (v: View) => void; tickets: MonitoredTicket[]; lotteryState:LotteryState; setLotteryState:React.Dispatch<React.SetStateAction<LotteryState>> }) {
  ...
  type Subscription={bank_id:string;monthly_amount_cents:number;currency:string;next_due_date:string|null;grace_days:number;status:string;last_paid_at:string|null};
  const [subscriptions,setSubscriptions]=useState<Subscription[]>([]);
  const [subAmount,setSubAmount]=useState("300");const [subDue,setSubDue]=useState("");const [subGrace,setSubGrace]=useState("0");
  ...
  const [scheduleOpen,setScheduleOpen]=useState(false);const [syncingSchedules,setSyncingSchedules]=useState(false);const [syncScheduleMessage,setSyncScheduleMessage]=useState("");type ScheduleSyncRow={lotteryId:string;name?:string;ok:boolean;drawTime?:string;closingTime?:string;nextDrawDate?:string;error?:string};const [scheduleSyncRows,setScheduleSyncRows]=useState<ScheduleSyncRow[]>([]);
  ...
  useEffect(()=>{void fetch("/api/gespro/subscriptions",{cache:"no-store"}).then(async r=>r.ok?await r.json() as Subscription[]:[]).then(setSubscriptions).catch(()=>{})},[]);
    ...
    <main className="admin-main">
      ...
      <section className="admin-content">
        ...
        {supportOwner ? <section className="panel bank-admin-panel"><Button variant="outline" onClick={()=>setSupportOwner(null)}><Localized text={"Sòti nan bank"}/></Button><h2>Lotri — {supportOwner.lottery}</h2><p>Konfigirasyon lotri ak peman.</p><LotteryManagement key={supportOwner.lottery} bank={supportOwner.lottery} isSuper={false} management={bankManagement} state={lotteryState} onChange={setLotteryState}/></section> : section==="Tableau de bord" ? <DashboardReport tickets={tickets} onCancel={onCancel}/> : section==="Mèt Bòlèt" ?
          <section className="panel owners-panel"><div className="panel-title"><div><p className="eyebrow">{owners.length} KONT</p><h2>Jesyon Mèt Bòlèt</h2></div></div><div className="owner-grid">{owners.map((owner)=><article key={owner.lottery} className="owner-card"><div className="owner-icon"><Store /></div><div><h3>{owner.lottery}</h3><p>{owner.name}</p></div><span className={`status ${owner.status==="Aktif"?"valide":"pèdi"}`}>{owner.status}</span><footer><span>{owner.sellers} vandè</span><button onClick={()=>setSupportOwner(owner)}>Antre nan bank →</button></footer>{(()=>{const sub=subscriptions.find(s=>s.bank_id==="e4637530-0d1c-472f-b1a4-fc14eed3a61e"&&owner.lottery==="mabooloto");return <div className="subscription-card"><strong>Lisans</strong>{sub ? (<><span>{(sub.monthly_amount_cents/100).toLocaleString(undefined,{style:"currency",currency:sub.currency})}/mwa</span><span>Next: {sub.next_due_date||"—"}</span><span>{sub.status.toUpperCase()}</span><Button className="admin-save-button" onClick={async()=>{const r=await fetch("/api/gespro/subscriptions",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({action:"paid",bankId:sub.bank_id,amountCents:sub.monthly_amount_cents})});if(r.ok){const updated=await r.json() as Subscription;setSubscriptions(v=>v.map(x=>x.bank_id===updated.bank_id?updated:x))}}}>PAID</Button></>) : (<><label>$/mwa<Input inputMode="decimal" value={subAmount} onChange={e=>setSubAmount(e.target.value)}/></label><label>Due date<Input type="date" value={subDue} onChange={e=>setSubDue(e.target.value)}/></label><label>Grace days<Input inputMode="numeric" value={subGrace} onChange={e=>setSubGrace(e.target.value.replace(/\D/g,"").slice(0,2))}/></label><Button className="admin-save-button" disabled={!subDue} onClick={async()=>{const r=await fetch("/api/gespro/subscriptions",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({action:"configure",bankId:"e4637530-0d1c-472f-b1a4-fc14eed3a61e",amountCents:Math.round(Number(subAmount)*100),dueDate:subDue,graceDays:Number(subGrace)||0})});if(r.ok){const created=await r.json() as Subscription;setSubscriptions(v=>[...v.filter(x=>x.bank_id!==created.bank_id),created])}}}>SAVE LISANS</Button></>)}</div>})()}</article>)}</div></section>
          : section==="Kès ak komisyon" ? <DashboardReport tickets={tickets} onCancel={onCancel}/> : section==="Paramèt" ?
          <section className="panel backend-panel"><LanguageLauncher/><div className="admin-sync-action"><Button className="admin-save-button" disabled={syncingSchedules} onClick={async()=>{setSyncingSchedules(true);setSyncScheduleMessage("");try{const r=await fetch("/api/gespro/sync-us-schedules",{method:"POST"});const b=await r.json() as {results?:ScheduleSyncRow[];error?:string};if(!r.ok)throw Error(b.error||"Sync pa pase.");const rows=b.results||[];setScheduleSyncRows(rows);const ok=rows.filter(x=>x.ok).length;setSyncScheduleMessage(`${ok}/${rows.length} orè konekte epi sove.`)}catch(e){setSyncScheduleMessage(e instanceof Error?e.message:"Sync pa pase.")}finally{setSyncingSchedules(false)}}}>{syncingSchedules?"SYNC...":"SYNC ORÈ API"}</Button>{syncScheduleMessage&&<span>{syncScheduleMessage}</span>}</div>{scheduleSyncRows.length>0&&<div className="table-scroll schedule-sync-table"><table className="bank-table"><thead><tr><th>Lotri</th><th>Lè API</th><th>Fèmen</th><th>Estati</th></tr></thead><tbody>{scheduleSyncRows.map(row=><tr key={row.lotteryId}><td><b>{row.name||row.lotteryId}</b></td><td>{row.drawTime||"—"}</td><td>{row.closingTime||"—"}</td><td><b className={row.ok?"ready":"pending"}>{row.ok?"✓ Sove":row.error||"Pa sove"}</b></td></tr>)}</tbody></table></div>}<div className="panel-title"><div><p className="eyebrow">FONDASYON SEKIRITE</p><h2>Backend GesPro</h2></div><span className={`connection-state ${backend?.configured?"online":"waiting"}`}>{backend?.configured?"Konekte":"Ap tann koneksyon"}</span></div><div className="backend-grid"><article><ShieldCheck /><div><h3>RLS ak wòl</h3><p>Separasyon kont bank, sipèvizè ak vandè aplike sou done lavant yo.</p></div><b><Localized text={"Aktive"}/></b></article><article><CircleDollarSign /><div><h3>Tranzaksyon atomik</h3><p>Limit nimewo verifye anvan tikè a valide.</p></div><b><Localized text={"Aktive"}/></b></article><article><CalendarClock /><div><h3>Fèmti tiraj</h3><p>Tiraj la fèmen otomatikman selon orè li.</p></div><b><Localized text={"Aktive"}/></b></article><article><Activity /><div><h3>Koneksyon Supabase</h3><p>{backend?.urlConfigured?"Adrès pwojè a antre.":"Bezwen URL ak kle pwojè Supabase la."}</p></div><b className={backend?.configured?"ready":"pending"}>{backend?.configured?"Konekte":"Poko konekte"}</b></article></div></section>
          ...
          : <section className="panel section-placeholder"><div className="placeholder-icon">{(()=>{const Icon=adminNav.find(([label])=>label===section)?.[1]??Activity;return <Icon />})()}</div><h2>{section}</h2><p>Pa gen done pou seksyon sa a kounye a.</p></section>}
      </section>
    </main>
  </div>;
}
...
export function PosInterface({ setView, tickets, setTickets, lotteryState, identity, remote, posLayout: layoutProp, serverOffsetMs=0 }: { remote?:{create:(plays:MonitoredTicket["plays"])=>Promise<MonitoredTicket>;cancel:(id:string)=>Promise<boolean>;pay?:(id:string)=>Promise<MonitoredTicket>}; identity?:{bank:string;pointId:string;seller:string}; posLayout?: PosLayout; serverOffsetMs?:number; lotteryState:LotteryState; setView: (v: View) => void; tickets: MonitoredTicket[]; setTickets: React.Dispatch<React.SetStateAction<MonitoredTicket[]>> }) {
  ...
  const [payTicket,setPayTicket]=useState<MonitoredTicket|null>(null);
  ...
  const posTickets=useMemo(()=>tickets.filter(t=>t.pointOfSaleId===posId),[tickets,posId]);
  ...
  function addPlay(amountOverride=amount) {
    ...
    if(!multiples&&saleLotteries.length===1&&availableLimit!==null&&cash>availableLimit){setNotice("Limit boul la rive. Jwèt la pa ajoute.");return}
    try {
  ...
  function menuAction(label:string){
    ...
    if(label==="Pay"){setPayCode("");setPayTicket(null);setAction("Pay");return}
    if(["Monitoring","Pending for payment","Sales history","Print report","Play monitor","View sales","Authorize strikeout"].includes(label)){setMonitorOpen(true);return}
