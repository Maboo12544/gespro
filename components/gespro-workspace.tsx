...
function RolePreview({ view, setView }: { view: View; setView: (v: View) => void }) {
  return <div className="role-preview" aria-label="Chanje entèfas pou aperçu"><button className={view==="admin"?"active":""} onClick={()=>setView("admin")}>Super Admin</button><button className={view==="pos"?"active":""} onClick={()=>setView("pos")}><Localized text={"Pwen Vant"}/></button></div>;
}
...
function AdminDashboard({ setView, tickets, onCancel, lotteryState, setLotteryState }: { onCancel: (id: string) => boolean; setView: (v: View) => void; tickets: MonitoredTicket[]; lotteryState:LotteryState; setLotteryState:React.Dispatch<React.SetStateAction<LotteryState>> }) {
  ...
  const [owners,setOwners] = useDemoState("gespro-workspace.tsx:owners",[
    { name:"Jean Pierre", lottery:"JP Bòlèt", sellers:12, status:"Aktif" },
    { name:"Marie Solange", lottery:"Solange Lotto", sellers:8, status:"Aktif" },
    { name:"Carlos Mentor", lottery:"CM Lottery", sellers:6, status:"Poz" },
  ]);
  ...
  return <div className="admin-shell">
    ...
    <main className="admin-main">
      <header className="admin-topbar"><div className="admin-mobile-brand"><Brand /></div><label className="search-box"><Search /><input aria-label="Rechèch" placeholder="Rechèch tikè, vandè, mèt bòlèt..." /></label><RolePreview view="admin" setView={setView} /><button className="icon-button" aria-label="Notifikasyon"><Bell /><span>3</span></button><div className="profile"><b>SA</b><span><strong>SUPER ADMIN</strong><small>Super Admin</small></span></div></header>
      <section className="admin-content">
        ...
        {supportOwner ? <section className="panel bank-admin-panel"><Button variant="outline" onClick={()=>setSupportOwner(null)}><Localized text={"Sòti nan bank"}/></Button><h2>Lotri — {supportOwner.lottery}</h2><p>Konfigirasyon lotri ak peman.</p><LotteryManagement key={supportOwner.lottery} bank={supportOwner.lottery} isSuper={false} management={bankManagement} state={lotteryState} onChange={setLotteryState}/></section> : section==="Tableau de bord" ? <DashboardReport tickets={tickets} onCancel={onCancel}/> : section==="Mèt Bòlèt" ?
          ...
          <section className="panel backend-panel"><LanguageLauncher/><div className="panel-title"><div><p className="eyebrow">FONDASYON SEKIRITE</p><h2>Backend GesPro</h2></div><span className={`connection-state ${backend?.configured?"online":"waiting"}`}>{backend?.configured?"Konekte":"Ap tann koneksyon"}</span></div><div className="backend-grid"><article><ShieldCheck /><div><h3>RLS ak wòl</h3><p>Separasyon kont bank, sipèvizè ak vandè poko aplike sou done lavant yo.</p></div><b><Localized text={"Pa aktive"}/></b></article><article><CircleDollarSign /><div><h3>Tranzaksyon atomik</h3><p>Limit nimewo verifye anvan tikè a valide.</p></div><b><Localized text={"Pa aktive"}/></b></article><article><CalendarClock /><div><h3>Fèmti tiraj</h3><p>Tiraj la fèmen otomatikman selon orè li.</p></div><b><Localized text={"Pa aktive"}/></b></article><article><Activity /><div><h3>Koneksyon Supabase</h3><p>{backend?.urlConfigured?"Adrès pwojè a antre.":"Bezwen URL ak kle pwojè Supabase la."}</p></div><b className={backend?.configured?"ready":"pending"}>{backend?.configured?"Konekte":"Poko konekte"}</b></article></div></section>
...
export function PosInterface({ setView, tickets, setTickets, lotteryState, identity, remote, posLayout: layoutProp }: { remote?:{create:(plays:MonitoredTicket["plays"])=>Promise<MonitoredTicket>;cancel:(id:string)=>Promise<boolean>}; identity?:{bank:string;pointId:string;seller:string}; posLayout?: PosLayout; lotteryState:LotteryState; setView: (v: View) => void; tickets: MonitoredTicket[]; setTickets: React.Dispatch<React.SetStateAction<MonitoredTicket[]>> }) {
  const posBank=identity?.bank??"JP Bòlèt",posId=identity?.pointId??"jp-pos-1",sellerName=identity?.seller??"Jean Pierre";
  const [monitorOpen,setMonitorOpen] = useState(false);
  const activeLotteries=lotteryState.items.filter(i=>i.resultMode!=="pending"&&(!i.bank||i.bank===posBank)&&!lotteryState.removed[JSON.stringify([i.id,"*","removed"])]&&!lotteryState.removed[JSON.stringify([i.id,posBank,"removed"])]);
  ...
  async function createTicket(){
    ...
    let serial="";
    do {serial=Array.from(crypto.getRandomValues(new Uint8Array(8)),n=>String(n%10)).join("")} while(tickets.some(t=>t.id===serial));
    const ticket: MonitoredTicket={id:serial,agentCode:"01",createdAt,bank:posBank,pointOfSaleId:posId,seller:sellerName,amount:Number(total.toFixed(2)),prize:0,status:"pending",cancelUntil:createdAt+5*60*1000,plays:plays.map(play=>({...play}))};
    setTickets(items=>[...items,ticket]);setPlays([]);setNotice("");
    setReceiptToPrint(ticket);
    return {status:"demo_created",id:ticket.id,total:ticket.amount};
  }
  function menuAction(label:string){
    ...
    if(label==="Duplicate"){
      const last=latestDuplicableTicket(tickets,posBank,posId);
      if(!last){setNotice("Pa gen tikè pou duplike.");return}
      if(last.plays.some(p=>!activeLotteries.some(l=>l.name===p.lottery))){setNotice("Tikè sa a gen lotri ki pa aktive.");return}
      try{appendPlays(last.plays.map(p=>({...p,id:nextId.current++})))}catch(error){setNotice(error instanceof Error?error.message:"Tikè a pa ka duplike.")}return
    }
    setAction(label);
  }
