...
export function ServerSalesDashboard({bank,canCancel}:{bank:string;canCancel:boolean}){
 ...
 return <section className="access-server-sales"><h2>Lavant senkronize</h2><p role="status">{data.message||(!data.ready?'Ap chaje tikè…':`Dènye rafrechisman: ${data.updated} · Chak 15 segonn`)} <button onClick={()=>void data.refresh()}>Rafrechi</button></p><DashboardReport tickets={data.tickets} onCancel={canCancel?data.cancel:()=>false} serverBacked canCancel={canCancel}/></section>
}
