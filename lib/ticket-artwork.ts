...
export function ticketArtwork(ticket:MonitoredTicket,copy:boolean):Artwork{
 ...
 center(`TOTAL: ${amount(ticket.amount)}`,70);rule();
 ...
 y+=10;
 const artwork:Artwork={width:W,height:y,svg:`<svg xmlns="http://www.w3.org/2000/svg" width="${W}" height="${y}" viewBox="0 0 ${W} ${y}"><rect width="100%" height="100%" fill="white"/><g fill="#111" font-family="DejaVu Sans Mono, Courier New, monospace" font-weight="700">${parts.join("")}</g></svg>`};
