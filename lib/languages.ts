...
Demonstrasyon • Done sesyon sa a sèlman.|Démonstration • Données de cette session uniquement.|Demo • Current session data only.|Demostración • Solo datos de esta sesión.
...
Lavant tès|Ventes de test|Test sales|Ventas de prueba
Pa gen tranzaksyon tès pou dat sa a.|Aucune transaction de test à cette date.|No test transactions for this date.|No hay transacciones de prueba para esta fecha.
VIEW SALES|VOIR LES VENTES|VIEW SALES|VER VENTAS
...
Demonstrasyon • Tikè sesyon sa a sèlman. Rafrechi paj la efase done tès yo. Lè yo itilize lè aparèy la.|Démonstration • Tickets de cette session uniquement. Actualiser efface les tests. Heure de l’appareil.|Demo • Session tickets only. Refreshing clears test data. Times use the device clock.|Demostración • Solo boletos de esta sesión. Actualizar borra los datos de prueba. Hora del dispositivo.
Balans ak tranzaksyon kès yo poko konekte. Lis anba a montre lavant tès yo sèlman.|Les soldes et transactions de caisse ne sont pas connectés. Seules les ventes de test figurent ici.|Cash balances and transactions are not connected. Only test sales appear here.|Los saldos y transacciones de caja no están conectados. Solo aparecen ventas de prueba.
